...
log "Applying migrations before reliability checks"
"${GOOSE_CMD[@]}" -dir db/migrations postgres "${GOOSE_DSN}" up \
  > "${OUT_DIR}/goose_up.txt" 2>&1

log "Running worker reliability unit tests (retry/backoff/dead-letter)"
go test ./internal/queue/worker \
  -run 'TestHandleFailure_SchedulesRetryWhenAttemptsRemain|TestHandleFailure_DeadLettersWhenAttemptsExhausted|TestHandleFailure_RescheduleFailureFallsBackToMarkFailed|TestProcessOne_NoClaimedJobReturnsFalseNil|TestExponentialBackoff_StaysWithinExpectedBounds|TestBuildRegistrationsCSV|TestHealthHandlerReadyz' \
  -v > "${OUT_DIR}/worker_reliability_unit_tests.txt" 2>&1

log "Running DB-backed worker pipeline integration tests"
go test ./internal/http/integration \
  -run 'TestPublishPipeline_EndToEnd|TestPipeline_Register_EnqueuesJob_Worker_SendsOnce|TestPipeline_RegistrationsCSVExport_EnqueueProcessDownload' \
  -v > "${OUT_DIR}/worker_reliability_integration_tests.txt" 2>&1
...
Day 90 worker reliability summary
- Worker reliability unit tests: passed
- Worker pipeline integration tests: passed
- Retry/dead-letter branches covered: passed
- TEST_DB_DSN used: ${TEST_DB_DSN}
- Duration seconds: ${duration}
EOF

log "Day 90 worker reliability checks completed successfully"
